...
exports.getSubscriptionStatus = onCall(
   async ({ data, auth }): Promise<SubscriptionStatusResponse> => {
      ...
      const userData = await getFirestore().collection('stripe_customers').doc(userId).get();
...
exports.createSubscrition = onCall<CreateSubscriptionRequest, any>(
   async ({ data, auth }): Promise<CreateSubscriptionResponse | any> => {
      ...
      try {
         ...
         if (!email) {
            return { success: false, result: 'Email and PaymentMethodId are required' };
         }
         ...
         const customerData = await getFirestore()
            ...
            .doc(auth.uid)
            .get();
         const { customer_id } = customerData.data() as { customer_id: string };
         ...
         if (existingSubs.data.length > 0 && existingSubs.data[0].status === 'active') {
         ...
         return { success: false, result: 'An error occurred while creating the subscription' };
      }
   }
);
...
exports.getPortalUrl = onCall(async ({ data, auth }): Promise<CreateSubscriptionResponse> => {
   try {
      if (!auth) return { success: false, result: 'No authorized' };
...
exports.handleStripeWebhook = onRequest(
   ...
   async (req: functions.https.Request, res: functions.Response): Promise<any> => {
