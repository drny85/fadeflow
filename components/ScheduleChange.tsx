...
const DayScheduleRow: React.FC<DayScheduleRowProps> = ({
    ...
    const handleTimeChange = (
        ...
        if (selectedDate) {
            const timeString = formatTime12Hour(
    ...
    return (
        <View className="my-1 w-full flex-row items-center bg-card p-2 shadow-sm">
            ...
            <Toggle
                ...
                onValueChange={(value) => onUpdate(day, 'isOff', value)}
            />
            {!schedule.isOff && (
                <View className="flex-grow items-center gap-2">
                    <View className="flex-row gap-3">
                        <View>
                            <Text className="mb-1 text-center text-muted">
                                Start
                            </Text>
                            <DatePicker
                                value={
                                    new Date(
                                        `1970-01-01T${parseTime12HourA(schedule.startTime)}`
                                    )
                                }
                                mode="time"
                                display="default"
                        ...
                        <View>
                            ...
                            <DatePicker
                                value={
                                    new Date(
                                        `1970-01-01T${parseTime12HourA(schedule.endTime)}`
                                    )
                                }
                                mode="time"
                                display="default"
                    ...
                    <View className="flex-row gap-3">
                        <DatePicker
                            value={
                                new Date(
                                    `1970-01-01T${parseTime12HourA(schedule.lunchBreak.start)}`
                                )
                            }
                            mode="time"
                            display="default"
                        ...
                        <DatePicker
                            value={
                                new Date(
                                    `1970-01-01T${parseTime12HourA(schedule.lunchBreak?.end)}`
                                )
                            }
                            mode="time"
                            display="default"
