...
const ScheduleComponent: React.FC = () => {
   ...
   const updateDaySchedule = (key: keyof ScheduleDay, value: string | LunchBreak) => {
   ...
   const onChangeTime = (event: any, selectedTime: Date | undefined, type: string) => {
      ...
      switch (type) {
         ...
         case 'lunchStart':
            if (formattedTime)
               updateDaySchedule('lunchBreak', { ...selectedDay.lunchBreak, start: formattedTime });
            setShowLunchStartPicker(false);
            break;
         case 'lunchEnd':
            if (formattedTime)
               updateDaySchedule('lunchBreak', { ...selectedDay.lunchBreak, end: formattedTime });
               ...
               {weekDays.map((day, index) => (
                  <TouchableOpacity key={index} onPress={() => handleDayPress(index)}>
                     <View
                        style={[
                           styles.dayButton,
                           index === selectedDayIndex && styles.selectedDay,
                        ]}>
            ...
            {!selectedDay.isOff ? (
               <View>
                  ...
                  <View className="flex-row items-center justify-evenly">
                     <Text className="font-roboto-bold text-xl">{selectedDay.startTime}</Text>
                     <DateTimePicker
                        value={addHours(startOfDay(new Date()), 8)}
                        mode="time"
                        minuteInterval={15}
                        is24Hour={false}
                        onChange={(event, date) => onChangeTime(event, date, 'start')}
                     />
                  </View>
                  ...
                  <View className="flex-row items-center justify-evenly">
                     ...
                     <DateTimePicker
                        ...
                        mode="time"
                        is24Hour={false}
                        onChange={(event, date) => onChangeTime(event, date, 'end')}
                     />
                  </View>
                  ...
                  <View style={styles.lunchContainer}>
                     ...
                     {showLunchStartPicker && (
                        <DateTimePicker
                           ...
                           mode="time"
                           is24Hour={false}
                           onChange={(event, date) => onChangeTime(event, date, 'lunchStart')}
                        />
                     )}
                     <TouchableOpacity
                     ...
                     {showLunchEndPicker && (
                        <DateTimePicker
                           ...
                           mode="time"
                           is24Hour={false}
                           onChange={(event, date) => onChangeTime(event, date, 'lunchEnd')}
                        />
                     )}
                  </View>
               </View>
